...
export def main [
    ...
    if ($path | bf fs is_not_dir) { bf write error $"Cron directory does not exist: ($path)." cron }

    # output running message
    if $quiet { bf write debug $"($path)." cron } else { bf write $"($path)." cron }

    # use run-parts to execute cron jobs
    { ^run-parts --exit-on-error $path } | bf handle cron | ignore
}
